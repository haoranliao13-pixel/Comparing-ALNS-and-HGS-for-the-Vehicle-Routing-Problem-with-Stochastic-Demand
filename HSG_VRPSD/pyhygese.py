...
# ===== helper: 函数式封装，兼容 test_with_both.py 的调用风格 =====
def solve_cvrp_pyhygese(
    dist_matrix=None,
    demand=None,
    Q=100.0,
    K=None,
    ...
    pars = AlgorithmParameters(timeLimit=time_limit, seed=seed)
    solver = Solver(pars, verbose=bool(verbose))

    if dist_matrix is None and coords is None:
        raise ValueError("dist_matrix 与 coords 至少提供其一")
    if demand is None:
        raise ValueError("必须提供 demand（长度 N+1，demand[0]=0）")
    ...
    if dist_matrix is not None:
        ...
        n = len(demand)
        data["x_coordinates"] = [0.0] * n
        data["y_coordinates"] = [0.0] * n
    else:
        xs = [x for x, _ in coords]
        ys = [y for _, y in coords]
        data["x_coordinates"] = xs
        data["y_coordinates"] = ys

    res = solver.solve_cvrp(data)
    routes = [list(map(int, r)) for r in res.routes]
    return routes, float(res.cost)

# 兼容别名（若 test_with_both.py 里导入的是 solve_cvrp）
def solve_cvrp(*args, **kwargs):
    return solve_cvrp_pyhygese(*args, **kwargs)
