...
class VRPSDInstance:
    ...
    def distance_matrix(self) -> np.ndarray:
        if self._D is None:
            a = self.coords[:, None, :]
            b = self.coords[None, :, :]
            self._D = np.sqrt(((a - b) ** 2).sum(axis=2))
        return self._D
...
def limit_routes_to_K(instance: VRPSDInstance, routes: Solution, K: Optional[int]) -> Solution:
    """If number of routes exceeds K, iteratively merge two routes.
    First rank candidate merges by geometric bridge delta, then (optionally) refine the best few with a tiny SAA to better reflect recourse costs.
    """
    if K is None or K <= 0:
        return routes
    ...
    while len(routes) > K:
        m = len(routes)
        # compute geometric deltas for all ordered pairs
        candidates = []
        for i in range(m):
            for j in range(m):
                if i == j: 
        ...
        try:
            ...
            for di, i, j, Ai, Bi, mode in shortlist:
                merged = Ai + Bi
                new_routes = []
                ...
                c = expected_cost_SAA(instance, new_routes, scen)
                if (c - base_cost) < best_delta:
                    best_delta = c - base_cost
                    best = (i, j, Ai, Bi)
            if best is not None:
                i, j, Ai, Bi = best
            else:
