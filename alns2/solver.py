...
def build_operators(alns: ALNS, distances: np.ndarray, py_rng: random.Random,
    ...
    try:
        from .operators import (
            ...
            removed_customers, greedy_insert, regret_k_insert, relocate_10, two_opt_star, or_opt2
        )
    except ImportError:
        from operators import (
            random_removal, worst_removal, shaw_removal,
    ...
    def _educate(routes):
        # distance-based quick local improvements; keep lightweight
        r = relocate_10(routes, distances, max_trials=200)
        r = two_opt_star(r, distances, max_trials=300)
        r = or_opt2(r, distances, max_trials=200)
        return r
    ...
    def repair_greedy(state: VRPSDState, _rng, **_kw):
        ...
        new_routes = _educate(new_routes)
        new_routes = enforce_K(new_routes)
